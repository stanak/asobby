...
- `/profile`: 本人のプロフィール編集。プレイヤーネームの直下にメインキャラクターを置く。下部には対象者のみ、一度限りのランク変更を折りたたんで表示する。ランク変更はプロフィール保存とは独立し、編集中の入力を保持して即時反映する。初回のランク選択はロビーで行う。
- `/players/{Discord ID}`: ユーザーページ。ロビーのホスト・同定済みゲストの「アイコン＋名前」全体がそれぞれリンクになる。未同定の人はリンクなし。
- `/players`: プレイヤー検索。検索条件はURLに保存し、ページ送りとブラウザの戻る操作に対応する。
- `/players/statistics`: 年齢層・ランク・国・デバイス・実際の使用キャラによる[集団統計](player-statistics.md)。
...
| `character_winrates_public` | **個人ページ**のキャラ別勝率を他ユーザーへ公開する許可。falseでも5人以上の集団統計には含む | `false` |
...
5人未満の集団は `suppressed: true` と空配列。区分内の少人数と逆算防止の補助区分は `count: null`。
年齢帯は10歳未満・10代〜90代・100歳以上。年齢は公開・非公開（統計利用許可）の情報を使う。
秘密の生年月日・空の居住国は、この旧形式の各配列から除外。個別の生年月日は返さない。
条件付き統計は `GET /api/players/analytics`。仕様は[プレイヤー統計](player-statistics.md)を参照。
