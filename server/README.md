...
- 初回選択とは別に、**アカウントごとに一度だけ** E / N / Ex / H / L へ手動変更できる。既存ユーザーも対象。**現在Phのユーザーは変更不可**で、Phを変更先に選ぶこともできない。「プロフィール編集」ページ下部の「ランクを変更する（1回のみ）」から確認後に実行する。初回選択は引き続きロビーで行い、手動変更はプロフィール保存とは独立して即時反映する（編集中の入力は保持する）
