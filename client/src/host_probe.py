...
def _autopunch_nat_port_on_socket(
    sock: socket.socket,
    host: str,
    port: int,
) -> Optional[int]:
    """AutoPunch リレーから NAT ポートを引く (lookup に使った socket をそのまま使う)。"""
...
def _probe_rtt_autopunch(
    ...
    *,
    ...
    """AutoPunch ホストへ RTT を測る (lookup と echo は同一 UDP socket 必須)。"""
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    try:
        sock.bind(("0.0.0.0", 0))
        nat_port = _autopunch_nat_port_on_socket(sock, host, port)
        if nat_port is None:
            return None
        ...
        for i in range(tries):
            ...
            reply = probe_host_once_on_socket(
                sock,
                host,
                nat_port,
                packet,
